...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val listOfEmptyCells = board.getAllCells().filter { board[it]==null }
        return when {
            listOfEmptyCells.isEmpty() -> null
            else -> listOfEmptyCells[listOfEmptyCells.indices.shuffled().first()] to generateRandomStartValue()
        }
    }
}